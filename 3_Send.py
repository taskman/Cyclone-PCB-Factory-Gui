...
if (showEtch):
	...
	currentlyEtching = "etch3"
elif (showDrill):
	currentlyEtching = "drill"
elif (showEdge):
	currentlyEtching = "edge"
...
def write_image():
	...
	if (storingFile == False):
		storingFile = True
		try:
			incompleteFileName = filePath + "/" + fileName + "_" + currentlyEtching + "_incomplete.png"
			completeFileName = filePath + "/" + fileName + "_" + currentlyEtching + ".png"
			gcodeviewer.savefig(incompleteFileName, dpi=600)
			
			#only use the incomplete file if the file size is large enough, sometimes a blank image is written
			fileSize = os.path.getsize(incompleteFileName)
			if (fileSize > (108844)) :			
				shutil.copy(incompleteFileName, completeFileName)
...
thread = Thread(target = write_image)
thread.start()
