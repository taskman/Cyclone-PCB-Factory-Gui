...
pltShowNonBlocking()
fig.savefig(filePath + "/" + fileName + "_Z_probing.png", dpi=600)
sys.stdout.flush()
...
interpolatedFig = plt.figure()
plt.pcolor(x_points, y_points, z_points)
plt.colorbar()
plt.title("Z probing results (interpolated) [mm]")
plt.axis('equal') # 1:1 aspect ratio
pltShowNonBlocking()
interpolatedFig.savefig(filePath + "/" + fileName + "_Z_probing_interpolated.png", dpi=600)
sys.stdout.flush()

cy.close() # Close the connection with Cyclone


print("Getting z origin")
execfile("2_Zprobe_Toolchange.py")
print("Finished getting z origin.  Please disconnect Z probing wiring")
